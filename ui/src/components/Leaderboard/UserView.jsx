...
function UserView({ userId, embedded = false }) {
  ...
  const shownBets = (player?.bets ?? [])
    .filter((bet) => isBefore(today, new Date(bet?.match?.date)) && bet?.bet)
  ...
  return (
    ...
    >
      <ContentContainer className=" h-full text-dec-background p-4 select-none" maxWidthClassName="max-w-5xl">
        {!embedded && (
            ...
            <thead>
              <tr className="border-b-4 border-dec-primary-light h-16 text-dec-h4 bg-white text-black rounded-md">
                ...
                <th className="text-left font-extrabold" data-tip="Points" data-for="Points" data-place="left">
                  {isSmall ? "Points" : "P"}
                  {!isSmall && <Tooltip id="Points" textColor="dec-primary" backgroundColor="white" />}
                </th>
              </tr>
            </thead>
            <tbody>
              {shownBets?.map((bet) => {
                ...
                return (
                  <tr className=" border-b-4 border-dec-primary-light" key={bet?.matchId}>
                    <td className="pl-2">
                    ...
                    <td className="py-4 space-y-2 sm:space-y-0 sm:flex sm:space-x-4">
                      <div className="">
                        <div className="flex space-x-2">
                          {match?.status?.long === "Match Finished" && (
                            <div className="flex items-center sm:space-x-2 space-x-1">
                              <div>FT</div>
                              <div>{match?.status?.long === "Match Finished" ? match?.score?.goals?.home : "-"}</div>
                              <div>:</div>
                              <div>{match?.status?.long === "Match Finished" ? match?.score?.goals?.away : "-"}</div>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {match?.status?.short === "AET" && (
                        <div className="flex items-center sm:space-x-2 space-x-1">
                          <div>ET</div>
                          <div className="w-10">
                            {Number(match?.score?.goals?.home) + Number(match?.score?.extraTime?.home)} :{" "}
                            {Number(match?.score?.goals?.away) + Number(match?.score?.extraTime?.away)}
                          </div>
                        </div>
                      )}
                    </td>

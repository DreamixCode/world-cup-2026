...
curl https://35-156-215-22.sslip.io/api/actuator/health
```
...
cd api
podman-compose -f docker-compose.prod.yml up -d --build api
```
...
- Node.js 20+
- AWS CLI configured with permissions to write to the S3 bucket and invalidate the CloudFront distribution
...
```bash
aws s3 sync dist/ s3://dreamix-worldcup-bucket --delete
```
